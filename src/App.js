...
const thumbnailObject = imagesList[index]
const thumbnail = thumbnailObject.imageUrl
...
class App extends Component {
  ...
  onClickEachImg = id => {
    if (thumbnailObject.id === id) {
      console.log('Hello')
    }
  }

  render() {
    const {clickedImgList, thumbnailUrl} = this.state

    const filteredImgList = this.getFilteredImages()

    console.log(filteredImgList)

    const score = clickedImgList.length
    const time = 0
    return (
      <div className="match-game-container">
        <nav className="match-game-navbar">
          ...
          />
          <div className="match-game-score-timer-container">
            <p className="match-game-score">
              Score: <span className="game-score">{score}</span>
            </p>
            <div className="match-game-timer-container">
              <img
                src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png"
                alt="timer"
                className="timer-image"
              />
              <p className="match-game-timer">{`${time} sec`}</p>
            </div>
          </div>
        </nav>
        <div className="match-game-pictures-container">
          <img src={thumbnailUrl} alt="match" className="thumbnail-image" />
          ...
          <ul className="images-list-container">
            {filteredImgList.map(eachImgDetails => (
              <CategoryItem
                key={eachImgDetails.id}
                eachImgDetails={eachImgDetails}
                onClickEachImg={this.onClickEachImg}
              />
            ))}
          </ul>
        </div>
